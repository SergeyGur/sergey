...
TEST(GenerateTest, GenerateArray) {
    ...
    int arr[n];
    gsm::generate::generate_array(arr, n, max_value);
...
TEST(GenerateTest, EmptyArray) {
    ...
    const int n = 0;
    const int max_value = 100;
    EXPECT_NO_THROW(gsm::generate::generate_array(arr, n, max_value));
}

TEST(SortTest, EmptyArray) {
    int arr[] = {};
    gsm::sortings::interes_sort(arr, 0);
    SUCCEED();
}

TEST(SortTest, SingleElement) {
    int arr[] = {5};
    gsm::sortings::interes_sort(arr, 1);
    EXPECT_EQ(arr[0], 5);
}
...
TEST(SortTest, Standart) {
    ...
    gsm::sortings::interes_sort(arr, 4);
    for (int i = 0; i < 4; ++i) {
...
TEST(IOTest, PrintArray) {
    ...
    gsm::io::print_array("Test Array:", arr, 3);
